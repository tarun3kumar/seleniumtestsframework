...
public class MarionetteCapabilitiesFactory implements ICapabilitiesFactory {
    ...
    public DesiredCapabilities createCapabilities(final DriverConfig webDriverConfig) {
        ...
        // Set MarionetteDriver for local mode
        if (webDriverConfig.getMode() == DriverMode.LOCAL) {
            String marionetteDriverPath = webDriverConfig.getChromeDriverPath();
            if (marionetteDriverPath == null) {
                try {
    ...
    public void handleExtractResources() throws IOException {
        ...
        if (!new File(dir + OSUtility.getSlash() + "wires.exe").exists()) {
            FileUtility.extractJar(dir, WebDriverExternalResources.class);
        }

        if (OSUtility.isWindows()) {
            System.setProperty("webdriver.gecko.driver", dir + "\\wires.exe");
        } else {
            System.setProperty("webdriver.gecko.driver", dir + "/wires");
